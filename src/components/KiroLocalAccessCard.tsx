...
import * as kiroLocalAccessService from '../services/kiroLocalAccessService';
import type {
  KiroLocalAccessState,
  KiroLocalAccessTestResult,
} from '../services/kiroLocalAccessService';
...
export function KiroLocalAccessCard({
  accounts,
  currentAccountId,
  maskAccountText,
  layoutMode,
  ...
  const [state, setState] = useState<KiroLocalAccessState | null>(null);
  ...
  const [testResult, setTestResult] = useState<KiroLocalAccessTestResult | null>(null);
  const [testDialogError, setTestDialogError] = useState('');
  const [portInput, setPortInput] = useState('');

  useEscClose(showPanel && !showTestDialog, () => setShowPanel(false));
  useEscClose(showTestDialog, () => {
    if (!testDialogRunning) {
  ...
  const refresh = useCallback(async (opts?: { silent?: boolean }) => {
    try {
      const nextState = await kiroLocalAccessService.getState();
      setState(nextState);
      setError('');
      if (!opts?.silent) {
        setNotice('');
  ...
  const accountMap = useMemo(() => {
    const map = new Map<string, KiroAccount>();
  ...
  const memberAccounts = useMemo(() => {
    ...
    return ids
      ...
      .filter((account): account is KiroAccount => Boolean(account));
  }, [accountMap, collection?.accountIds]);

  const missingMemberCount = Math.max(
    0,
    (collection?.accountIds?.length ?? 0) - memberAccounts.length,
  );
  ...
  const summaryStats = useMemo(
    () => [
      ...
      },
      {
        ...
        detail: missingMemberCount > 0 ? `另有 ${missingMemberCount} 个账号已失效待同步` : '账号池与当前列表一致',
      },
      {
        key: 'latency',
        label: '平均延迟',
  ...
  const syncAllAccounts = useCallback(async () => {
    ...
    setNotice('');
    try {
      const nextState = await kiroLocalAccessService.saveAccounts(accounts.map((account) => account.id));
      setState(nextState);
  ...
  const handleToggle = useCallback(async () => {
    ...
    try {
      ...
      let nextState = await kiroLocalAccessService.setEnabled(nextEnabled);
      if (nextEnabled && accounts.length > 0) {
        nextState = await kiroLocalAccessService.saveAccounts(accounts.map((account) => account.id));
      }
      setState(nextState);
      setNotice(
        nextEnabled
  ...
  const handleRotateKey = useCallback(async () => {
    ...
    setNotice('');
    try {
      const nextState = await kiroLocalAccessService.rotateApiKey();
      setState(nextState);
  ...
  const handleClearStats = useCallback(async () => {
    ...
    setNotice('');
    try {
      const nextState = await kiroLocalAccessService.clearStats();
      setState(nextState);
      setNotice('服务统计已清空');
    } catch (err) {
      setError(formatError(err));
  ...
  const handleUpdatePort = useCallback(async () => {
    ...
    setNotice('');
    try {
      const nextState = await kiroLocalAccessService.updatePort(nextPort);
      setState(nextState);
  ...
  const handleRemoveAccount = useCallback(async (accountId: string) => {
    ...
    setNotice('');
    try {
      const nextState = await kiroLocalAccessService.removeAccount(accountId);
      setState(nextState);
  ...
  const runTest = useCallback(async () => {
    ...
    setTestDialogError('');
    try {
      const result = await kiroLocalAccessService.test();
      setTestResult(result);
      await refresh({ silent: true });
    } catch (err) {
      setTestDialogError(formatError(err));
    } finally {
      setTesting(false);
  ...
  const testSuccessResult = testResult && !testResult.failure ? testResult : null;
  const testFailure = testResult?.failure ?? null;
  ...
  const footerHint = running
    ? '监听 127.0.0.1，本地 OpenAI 兼容网关已就绪'
    : '启用后提供 /v1/models 与 /v1/chat/completions';
  const serviceModeLine = collection
    ? `账号池：${state?.memberCount ?? 0} 个账号 · 调用方式：kiro-cli 串行`
    : '启用后可同步当前 Kiro 账号池';
  ...
  const summaryAccountPresentation = previewAccounts[0]
    ? buildKiroAccountPresentation(previewAccounts[0], t)
    : null;
  ...
  return (
    <>
        ...
        <div className="folder-inline-header codex-local-access-header">
          <div className="codex-local-access-summary-trigger" style={{ cursor: 'default' }}>
            ...
            <div className="folder-inline-info">
              ...
              <span className="folder-inline-count">Kiro 本地 OpenAI 兼容入口</span>
            </div>
          </div>
        ...
        {summaryAccountPresentation && (
          <div className="codex-local-access-pool-row" aria-label="账号池摘要">
            <div className="codex-local-access-pool-pill">
              ...
              <span>
                {summaryPrimaryQuota?.label ?? 'Prompt'} {summaryPrimaryQuota?.valueText ?? '--'}
              </span>
      ...
      {showPanel && (
        ...
        >
          <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="kiro-local-access-panel-title"
          ...
          >
            <div className="modal-header codex-local-access-modal-header">
              <div className="codex-local-access-header-main">
                <div className="group-account-picker-title">
                  <Server size={20} />
                  <span id="kiro-local-access-panel-title">Kiro API 服务面板</span>
                </div>
            ...
            <div className="modal-body codex-local-access-modal-body">
              ...
              <div className="codex-local-access-inline-info">
                <CircleAlert size={14} />
                <span>当前实现是单请求串行模式。每次请求会临时切换本机 `kiro-cli` 登录态，请避免同时手动使用 `kiro-cli`。</span>
              </div>
                    ...
                    <>
                      <div className="codex-local-access-config-grid">
                        ...
                        <div className="codex-local-access-config-card codex-local-access-config-card-port codex-local-access-port-card">
                          <div className="codex-local-access-config-head">
                            <label className="codex-local-access-config-label" htmlFor="kiro-local-access-port">
                              监听端口
                            </label>
                          </div>
                          <div className="codex-local-access-port-row">
                            <input
                              id="kiro-local-access-port"
                              type="number"
                              min={1}
                              max={65535}
                              value={portInput}
                      ...
                      <div className="codex-local-access-config-extra-grid">
                        ...
                        <div className="codex-local-access-config-card codex-local-access-config-card-root">
                          <div className="codex-local-access-config-head">
                            <span className="codex-local-access-config-label">执行方式</span>
                          </div>
                          <code className="codex-local-access-code" title="通过 kiro-cli 串行转发">
                            通过 kiro-cli 串行转发
                          </code>
                        </div>
                      </div>
      ...
      {showTestDialog && (
        <div className="modal-overlay codex-local-access-test-dialog-overlay" onClick={closeTestDialog}>
          <div
            className="modal codex-local-access-test-dialog"
            role="dialog"
            aria-modal="true"
            aria-labelledby="kiro-local-access-test-dialog-title"
            onClick={(event) => event.stopPropagation()}
          >
            <div className="modal-header codex-local-access-test-dialog-header">
              <div>
                <h3 id="kiro-local-access-test-dialog-title">
                  <ShieldCheck size={18} />
                  <span>测试 API 服务</span>
                </h3>
                <p>向 Kiro 本地 API 服务发起一次真实请求，验证网关、密钥和 `kiro-cli` 响应。</p>
              </div>
              <button
            ...
            <div className="modal-body codex-local-access-test-dialog-body">
              {testDialogRunning && (
                <div className="codex-local-access-test-progress" aria-live="polite">
                  ...
                  <div>
                    ...
                    <span>正在向本地 API 服务发起真实请求，请稍候。</span>
                  </div>
                </div>
              )}
                ...
                <>
                  {testSuccessResult.output && (
                    <div className="codex-local-access-test-output">
                      <div className="codex-local-access-test-section-title">返回结果</div>
                      <pre>{testSuccessResult.output}</pre>
                    </div>
                  )}
                  <div className="codex-local-access-test-result">
                    <div className="codex-local-access-test-section-title">服务检测结果</div>
                    <div className="codex-local-access-inline-success">
                      <Check size={14} />
                      <span>
                        服务检测成功：本地服务、密钥和上游响应正常（{testSuccessResult.modelId ?? '--'}，
                        {formatLatencyMs(testSuccessResult.latencyMs ?? 0)}）。
                      </span>
                    </div>
                  </div>
                </>
              ...
              {testFailure && (
                <div className="codex-local-access-test-failure" aria-live="assertive">
                  ...
                  {testFailure.detail && (
                    <div className="codex-local-access-test-output">
                      <div className="codex-local-access-test-section-title">诊断详情</div>
            ...
            <div className="modal-footer codex-local-access-test-dialog-footer">
              <button className="btn btn-secondary" onClick={closeTestDialog} disabled={testDialogRunning}>
                {t('common.close', '关闭')}
              </button>
              <button className="btn btn-primary" onClick={() => void runTest()} disabled={testDialogRunning}>
                {testDialogRunning ? '测试中' : '重新测试'}
              </button>
            </div>
          </div>
        </div>
