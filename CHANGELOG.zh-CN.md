...
### 新增
- **Codex 现支持 Windows 多实例管理**：Codex 实例可在 Windows 上使用隔离的数据目录启动，并保留账号绑定、进程跟踪与官方本地状态文件中的会话可见性处理。
- **Codex 本地 API 服务现支持多个受管服务**：可创建、重命名、选择、启停、删除并激活独立 API 服务，每个服务都有自己的上游账号、API Key、默认密钥、健康检查与用量统计。
- **Gemini 默认实例凭据现可同步到 WSL**：设置项与快捷控制可把默认 Gemini 凭据文件同步到 WSL 工作流使用的位置。

### 变更
- **托盘空闲路径现会卸载主 WebView**：关闭到托盘时可销毁主 WebView，并在需要时恢复窗口，降低空闲内存占用且保留托盘操作。
- **Codex 与 GitHub Copilot 账号卡片进一步共用布局样式**：账号卡片接入通用卡片外壳类，同时保留各平台自己的操作与配额展示。
- **Codex 账号展示现可识别 Business 与 Codex Seat 套餐值**：Business 类套餐字符串会展示为更清晰的套餐标签，不再直接回退为原始值。

### 修复
- **启动时会 checkpoint WebKit LocalStorage WAL 文件**：应用启动维护 WebKit LocalStorage 数据库，避免重复运行后 WAL 文件无限增长。
- **删除 Codex 账号时会清理唤醒任务中的失效引用**：账号删除后会移除相关唤醒任务引用，并删除已没有账号的任务。
- **全局删除标签现使用应用内确认弹框**：失败会停留在当前弹框内提示，确认文案也已补齐多语言。
